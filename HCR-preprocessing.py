...
for file in os.listdir(file_path):
    if file.endswith('.czi'):
        ...
        embryo_name, sig_ch1_name, sig_ch2_name, ref_ch_name = re.split(r'_ch\d_', file)
        ...
        print(f'Creating Reference Channel nrrd file with name : {embryo_name}_{fish_num}_{ref_ch_name}.nrrd ')
        nrrd.write(os.path.join(ref_channel_path, f"{embryo_name}_{fish_num}_{ref_ch_name}.nrrd"), RImage,
                   index_order='C')
        print(f'Creating Signal1 Channel nrrd file with name : {embryo_name}_{fish_num}_{sig_ch1_name}.nrrd')
        nrrd.write(os.path.join(sig_channel_path, f"{embryo_name}_{fish_num}_{sig_ch2_name}.nrrd"), S1Image,
                   index_order='C')
        print(f'Creating Signal1 Channel nrrd file with name : {embryo_name}_{fish_num}_{sig_ch1_name}.nrrd')
        nrrd.write(os.path.join(sig_channel_path, f"{embryo_name}_{fish_num}_{sig_ch2_name}.nrrd"), S2Image,
                   index_order='C')
